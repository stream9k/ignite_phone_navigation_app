...
- **최종 동작 자동화**: 앱 종료 후, 설정된 대기 시간(분 단위)이 지나면 비행기 모드와 시스템 종료가 각각 독립적으로 실행됩니다.
  1. **비행기 모드**: 네트워크를 차단하여 대기 전력을 최소화합니다. (다음에 시동 걸면 비행기 모드 자동 해제)
      - **설정 독립화**: 전원 해제 시점부터 설정한 '분' 후에 독립적으로 실행됩니다.
  ...
  2. **시스템 종료**: 기기 전원을 완전히 끕니다. (장기 주차 시 배터리 방전 방지)
      - **설정 독립화**: 전원 해제 시점부터 설정한 '분' 후에 독립적으로 실행됩니다.
...
### 다. 지연 시간 및 동작 설정 (독립 블록)

1. **앱 종료 블록**: 시동이 꺼진 후 앱 종료 시도 전 대기 시간(초)을 설정합니다.
2. **비행기 모드 블록**: 비행기 모드 자동 실행 스위치를 켜고, 동작 대기 시간(분)을 설정합니다.
3. **시스템 전원 종료 블록**: 시스템 전원 자동 종료 스위치를 켜고, 동작 대기 시간(분)을 설정합니다.
